use tpuf_v1::{WalBuffer, WalEntry, buffer_write, buffer_flush, Manifest, query, Vector, serialize_wal, deserialize_wal, compact, AppState, handle_write, handle_query};
use uuid::Uuid;
use std::sync::Arc;
...
async fn main() {
    ...
    let state = Arc::new(AppState {
        buf: tokio::sync::Mutex::new(WalBuffer {
            next_seq: 0,
            pending: Vec::new(),
        }),
    ...
    let app = axum::Router::new()
        .route("/write", axum::routing::post(handle_write))
